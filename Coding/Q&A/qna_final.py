...
FALLBACK_SYSTEM_PROMPT = (
    "You are an expert assistant. Answer the user's question directly and accurately. "
    "If no context is provided, use your own knowledge. "
    "Do NOT say 'Not specified in the provided context.'"
)
...
def ask_openai(prompt: str, *, allow_fallback: bool = False) -> str:
    """
    allow_fallback = False → use strict context-limited system prompt
    allow_fallback = True  → ignore context, answer with general knowledge
    """
    try:
        sys_prompt = FALLBACK_SYSTEM_PROMPT if allow_fallback else SYSTEM_PROMPT
...
def main():
    ...
    while True:
        ...
        if not docs:
            print("[debug] No relevant context found. Using fallback LLM answer.\n")
        ...
        if answer.strip().lower() == "not specified in the provided context.":
            print("[debug] Context not helpful, falling back to general LLM answer.\n")
            answer = ask_openai(q, allow_fallback=True)

        print("\nA>", answer if answer else "No answer returned.")
        print("   citations:", ids)
